"""
Chat route — persistent sessions, AI-powered, with clear error surfacing.
"""
...
async def chat_send(req: ChatRequest, db: Session = Depends(get_db)):
    # Check AI config before spending time on DB queries
    ai = get_provider_status()
    if not ai["ready"]:
        raise HTTPException(
            status_code=503,
            detail=(
                ...
                "Go to your Railway dashboard → Variables and add "
                "OR_KEY (OpenRouter) or DEEPSEEK_API_KEY (DeepSeek)."
            ),
        )
    ...
    try:
        ...
        # Persist only after a successful AI response
